...
wpas_pr_usd_elems(struct wpa_supplicant *wpa_s, const u8 *src)
{
	return NULL;
}
#endif /* CONFIG_PR */
					 ...
					 int deinit)
{
	...
	wpa_printf(MSG_DEBUG, "NAN: Start listen on %u MHz for %u ms",
		   lwork->freq, duration);
	/* remain-on-channel wrapper does not yet accept a forced
	 * address; retain the upstream address-filter state for later support. */

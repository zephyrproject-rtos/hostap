				...
				union wpa_event_data *data)
{
	...
	if (data) {
		...
		if (event == EVENT_AUTH) {
			...
			data_tmp->external_auth.ssid_len = data->external_auth.ssid_len;
			data_tmp->external_auth.action = data->external_auth.action;
			data_tmp->external_auth.key_mgmt_suite = data->external_auth.key_mgmt_suite;
		}
	}
...
int wpa_drv_zep_send_external_auth_status(void *priv,
	...
	if (!dev_ops || !dev_ops->send_external_auth_status) {
		wpa_printf(MSG_ERROR, "%s: send_external_auth_status op not supported",
			   __func__);
		goto out;
	}
	...
	if (ret) {
		wpa_printf(MSG_ERROR, "%s: send_external_auth_status op failed: %d", __func__, ret);
		goto out;
	}
	ret = 0;
out:
